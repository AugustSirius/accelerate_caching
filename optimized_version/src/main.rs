...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (default_data_folder, lib_file_path, report_file_path) = if is_macos {
        ...
        (
            ...
            "/Users/augustsirius/Desktop/DIA_peak_group_extraction/输入数据文件/raw_data/TPHPlib_frag1025_swissprot_final_all_from_Yueliang.tsv",
            "/Users/augustsirius/Desktop/DIA_peak_group_extraction/输入数据文件/raw_data/report.parquet"
        )
    } else {
    ...
    // Create optimized cache manager with parallel threads support
    let cache_manager = CacheManager::with_threads(parallel_threads);
    
    println!("\n========== DATA PREPARATION PHASE ==========");
    println!("Using optimized cache system with {} threads", parallel_threads);
    ...
    let (ms1_indexed, ms2_indexed_pairs) = if cache_manager.is_cache_valid(d_path) {
        ...
        println!("Using parallel loading with compression and memory mapping");
        ...
        println!("Optimized cache loading time: {:.5} seconds", cache_load_start.elapsed().as_secs_f32());
        result
    } else {
        ...
        // Save to optimized cache with parallel processing and compression
        println!("\nSaving to optimized cache with parallel sharding and compression...");
        let cache_save_start = Instant::now();
        cache_manager.save_indexed_data(d_path, &ms1_indexed, &ms2_indexed_pairs)
            .map_err(|e| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) })?;
        println!("Optimized cache saving time: {:.5} seconds", cache_save_start.elapsed().as_secs_f32());
    ...
    let max_precursors = 800; // 可以根据需要调整
